...
<body>
	<table border=1>
		<tr>
			<td>제목</td>
			<td colspan="6"><%=comunicationDTO.getTitle() %></td>
		</tr>
		<tr>
			<td>작성자</td>
			<td><%=comunicationDTO.getMemName() %></td>
			<td>작성일</td>
			<td colspan="1"><%=sf.format(comunicationDTO.getWriteDate()) %></td>
			<td>조회수</td>
			<td><%=comunicationDTO.getReadCount() %></td>
		</tr>
		<tr>
			<td colspan="7">
				<pre><%=comunicationDTO.getContent() %></pre>
			</td>
		</tr>
		<tr>
...
%>
		...
		<tr>
			<td colspan="6">
				<textarea name = "comment" id="comment" cols="40" rows="8"></textarea><br>
			</td>
			<td>
...
%>						
							...
							<input type="button" id="delete" name="delete" value="삭제" onclick="commentDelete('<%=commentsDTO.getNum()%>','<%=num%>','<%=pageNum%>','<%=listPageNum%>','<%=listCount%>')">
<%
						}
						if(session.getAttribute("member") != null){
%>						
...
%>
						<br>
						<pre><%=commentsDTO.getComment() %></pre>
						<br>
						</td>
<%
...
%>				
							<tr>
								...
								<td colspan="5">
									<textarea name = "commentReply" id="commentReply" cols="40" rows="8"></textarea><br>
								</td>
								<td>
...
%>
				<tr>
					...
					<td colspan="5">
						<textarea name = "commentModify" id="commentModify" cols="40" rows="8"><%=commentsDTO.getComment() %></textarea>	
					</td>
					<td>
